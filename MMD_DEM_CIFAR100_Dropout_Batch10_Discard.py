...
class LifeLone_MNIST(object):
    ...
    def SelectComponentByData_AfterDiscard(self,data):
        mycount = int(np.shape(data)[0] / self.batch_size)
        losses = []
    ...
    def Calculate_Acc_AfterDiscard(self,test,testLabel):
        ...
        accCount = 0
        totalCount = 0
        index = self.SelectComponentByData_AfterDiscard(test)
        ...
        for i in range(count):
            ...
            prediction = self.discrad_prediction[index].predict(batch)
            prediction = np.argmax(prediction,axis=1)
            #prediction = self.sess.run(self.predictionArr[index], feed_dict={self.input_test: single})
            for tt in range(self.batch_size):
                if prediction[tt] == baatchLabel[tt]:
    ...
    def Calculate_Acc2_AfterDiscard(self):
        totalAcc = 0
        for i in range(20):
            test = self.arr1_testing[i]
            testy = self.arr2_testing[i]
            ...
            acc = self.Calculate_Acc_AfterDiscard(test,testy)
            totalAcc = totalAcc + acc
        totalAcc = totalAcc / 20
        return totalAcc
    ...
    def train(self):
        ...
        with tf.Session(config=config) as sess:
            ...
            remainCount = self.componentCount - 6
            self.discrad_desireNum = remainCount

            self.Discard()

            acc = self.Calculate_Acc2_AfterDiscard()
