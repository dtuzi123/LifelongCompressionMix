...
class LifeLone_MNIST(object):
    def __init__(self):
        ...
        self.data_textX = np.concatenate((self.arr1_test,self.arr2_test,self.arr3_test,self.arr4_test,self.arr5_test),axis=0)
        self.data_textY = np.concatenate((self.labelArr1_test,self.labelArr2_test,self.labelArr3_test,self.labelArr4_test,self.labelArr5_test),axis=0)
    ...
    def Generate_GraphRelationship(self, generatorArr, componentArr):
        ...
        for i in range(ComponentCount):
            ...
            for j in range(i + 1, ComponentCount):
                ...
                mse1 = self.FID_Evaluation(set1, set2)
                mse1 = float(mse1)
                graph.append(mse1)
    ...
    def Make_Choice(self, index1, index2, componentArr, GeneratorArr):
        ...
        for i in range(np.shape(otherSet)[0]):
            tSet = otherSet[i]
            mse1 = self.FID_Evaluation(set1, tSet)
            sum1 = sum1 + mse1

            mse2 = self.FID_Evaluation(set2, tSet)
            sum2 = sum2 + mse2
    ...
    def Discard(self):
        ...
        for t1 in range(self.discrad_desireNum):
        ...
        for j in range(np.shape(GeneratorArr)[0]):
            ...
            self.discrad_prediction.append(TeacherPredictions[j])

    def SelectComponentBySingle_AfterDisard(self,single):
        x_test = np.tile(single,(self.batch_size,1))
        lossArr = []
        for i in range(np.shape(self.discard_vaeloss)[0]):
            loss = self.sess.run(self.discard_vaeloss[i],feed_dict={self.inputs:x_test})
            lossArr.append(loss)
        index = np.argmin(lossArr)
        return x_test,index

    def SelectComponentByBatch_AfterDiscard(self,batch):
        losses = []
        for i in range(np.shape(self.discard_vaeloss)[0]):
            loss1 = self.sess.run(self.discard_vaeloss[i], feed_dict={self.inputs: batch})
            losses.append(loss1)
    ...
    def Calculate_Accuracy_ByALL_AfterDiscard(self, testX,testY, index):
        ...
        for i in range(totalBatchCount):
            ...
            index = self.SelectComponentByBatch_AfterDiscard(batch)
            #pred = self.classifierModelArr[index].predict(batch)
            pred = self.discrad_prediction[index].predict(batch)
    ...
    def Create_VAEs(self,index,selectedComponentIndex):
        ...
        if self.componentCount == 0:
            ...
            self.codeArr.append(continous_variables1_sub)
    ...
    def Calculate_MixutreLoss_Single(self,single):
        batch = np.tile(single,(self.batch_size,1,1,1))
        lossValues = 0
    ...
    def train(self):
        ...
        with tf.Session(config=config) as sess:
            ...
            totalTrainingSteps = int(np.shape(self.totalSet)[0] / self.data_stream_batch)
            totalTrainingSteps = int(totalTrainingSteps / 20)
                    ...
                    if np.shape(self.FixedMemory)[0] > self.maxMmeorySize:
                        ...
                        if self.componentCount == 1:
                            ...
                            for kk in range(self.componentCount - 1):
                                stuPro = self.codeArr[self.componentCount - 1]
                                ...
                                for i in range(mycount2):
                                    ...
                                    code1 = self.sess.run(stuPro, feed_dict={self.inputs: generatedImages})
                                    code2 = self.sess.run(stuPro, feed_dict={self.inputs: newBatch})

                                    if np.shape(code1Arr)[0] == 0:
                                        code1Arr = code1
                                    else:
                                        code1Arr = np.concatenate((code1Arr, code1), axis=0)

                                    if np.shape(code2Arr)[0] == 0:
                                        code2Arr = code2
                                    else:
                                        code2Arr = np.concatenate((code2Arr, code2), axis=0)

                                code1Arr = np.array(code1Arr)
                                code2Arr = np.array(code2Arr)

                                mmd = compute_mmd(code1Arr, code2Arr)
                                dhsicArr.append(mmd)
                            ...
                            if threshold < dhsic:
                                ...
                                socreArr = []
                                for k in range(totalCount2):
                                    single = self.FixedMemory[k]
                                    single = np.reshape(single, (1, 32, 32, 3))
                                    singleScore = self.Calculate_MixutreLoss_Single(single)
                                    socreArr.append(singleScore)
                                socreArr = np.array(socreArr)
                                socreArr = socreArr * -1
                                sortIndex = socreArr.argsort()
                                self.FixedMemory = self.FixedMemory[sortIndex]
                                self.FixedMemoryLabel = self.FixedMemoryLabel[sortIndex]
            ...
            self.discrad_desireNum = self.componentCount - 10
            ...
            remainCount = self.componentCount - 10
            self.discrad_desireNum = remainCount

            acc = self.Calculate_Accuracy_ByALL_AfterDiscard(self.data_textX, self.data_textY, 0)
            print("acc after discard")

            print(np.shape(self.discard_vaeloss)[0])
