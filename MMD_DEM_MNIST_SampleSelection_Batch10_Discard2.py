...
class LifeLone_MNIST(object):
    def __init__(self):
        ...
        self.discard_vaeGeneration = []
        self.discard_disirNum = 7
    ...
    def Generate_GraphRelationship(self, generatorArr, componentArr):
        ...
        showGraph = np.zeros((self.componentCount,self.componentCount))
        dataCount = 20
        for kk in range(ComponentCount):
            ...
            for k1 in range(dataCount):
                ...
                generatedImages = np.reshape(generatedImages, (self.batch_size, 28, 28, 1))
    ...
    def Discard(self):
        ...
        for i in range(ComponentCount):
            ...
            TeacherPredictions.append(self.TeacherPredictions[i])
        ...
        for j in range(np.shape(GeneratorArr)[0]):
            ...
            self.discard_vaeGeneration.append(GeneratorArr[j])
    ...
    def Calculate_Accuracy_ByALL_AfterDiscard(self, testX,testY, index):
        ...
        for i in range(totalCount):
            ...
            x,index = self.SelectComponentBySingle_AfterDisard(single)
            pred = self.sess.run(self.discrad_prediction[index],feed_dict={self.inputs:x})
            pred = pred[0]
            myPro.append(pred)
    ...
    def train(self):
        ...
        with tf.Session(config=config) as sess:
            ...
            RemainCount = self.componentCount - 5
            self.discard_disirNum = RemainCount

            self.Discard()

            print("before the discard")
            print(self.componentCount)
            print(acc)

            for i in range(np.shape(self.discard_vaeGeneration)[0]):
                batch_z = np.random.uniform(-1, 1, [self.batch_size, self.z_dim]).astype(np.float32)
                mygan = self.sess.run(self.discard_vaeGeneration[i],feed_dict={self.z:batch_z})
                ims_cv_255("MMD_MNIST" + str(i) + ".png",mygan)
